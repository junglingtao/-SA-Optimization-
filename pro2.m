...
%% 加入随机性
yearss=["2024","2025","2026","2027","2028","2029","2030"];
filename1 = 'result2.xlsx';
filename2 = 'result1_2.xlsx';
for t=1:7 % 年份
    r2=readtable(filename2,"Sheet",yearss(t),"VariableNamingRule","preserve");
    r2=table2array(r2(1:82,3:43));
    for i=1:82
        for j=1:41
            temp=r2(i,j)+r2(i,j)*((rand - 0.5) * 0/100);
            if temp>r2(i,j)
                disp('no')
            else
                r2(i,j)=temp;
    ...
    Range = ['C2',':','AQ83'];
    xlswrite(filename1,r2, yearss(t), Range);
end
%% 最优种植方案写入excel
...
for i=1:7
    ...
    Range2 = ['C56',':','AQ83'];
    % 将矩阵写入 Excel 文件的特定位置
    xlswrite(filename2,first, sheet(i), Range1);
    xlswrite(filename2,second, sheet(i), Range2);
end
